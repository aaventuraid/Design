...
export async function getSettings(): Promise<AppSettings> {
  try {
    // Try to get settings from database first (preferred for dynamic config)
    const { DatabaseService } = await import('@/lib/database');
    const dbSettingsObj = await DatabaseService.getSettings();

    // Fallback to file-based settings for missing keys
    const candidates = getDataDirs().map((d) => path.join(d, 'settings.json'));
    const raw = await readFirstExisting(candidates);
    const fileSettings = raw ? JSON.parse(raw) : {};
